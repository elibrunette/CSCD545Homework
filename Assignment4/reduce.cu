...
//sharedN is the number of elements in the shared memory
__global__ void reduce3(float *in, float *out, int n, int sharedN)
{
    extern __shared__ float sdata[];
...
void runCUDA2( float *arr, int  n_old, int tile_width)
{    
   // set up host memory
   float *h_in, *h_out, *d_in, *d_out;
   int n = 0;
   int sharedSize = 0;   int sharedN = 0;
   h_in = GPUPrep(arr, n_old, &n); //Make input size power of 2.
   h_out = (float *)malloc(MAXDRET * sizeof(float));
   memset(h_out, 0, MAXDRET * sizeof(float));
   ...
   while( 1 )
   {
       reduce3<<<grid, block, sharedSize>>>(d_in, d_out, num_in, sharedN);
       check_cuda_errors(__FILE__, __LINE__);
       cudaDeviceSynchronize();
